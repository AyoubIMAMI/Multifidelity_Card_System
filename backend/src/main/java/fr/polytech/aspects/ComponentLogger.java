...
public class ComponentLogger {
    ...
    @Pointcut("execution(public * fr.polytech.components..*(..))")
    private void allComponentMethods() {
    } // This enables to attach the pointcut to a method name we can reuse below

    @Before("allComponentMethods()")
    ...
    @AfterReturning(pointcut = "allComponentMethods()", returning = "resultVal")
    ...
    @AfterThrowing(pointcut = "allComponentMethods()", throwing = "exception")
